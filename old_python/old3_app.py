...
# ------------------------------
def validate_schemas(openai_client, schema1: str, schema2: str) -> str:
    """Compare two schemas and recommend the best one"""
    response = openai_client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {
...
Your job is to compare two CREATE TABLE schemas and determine which one better follows star schema best practices.
...
Provide a clear recommendation with specific reasons."""
            },
            {
                "role": "user",
...
qdrant, openai_client = load_clients()
...
with tab2:
    ...
    if st.button("Validate & Compare", key="validate_btn"):
        if schema1.strip() and schema2.strip():
            with st.spinner("Analyzing schemas..."):
                validation_result = validate_schemas(openai_client, schema1, schema2)
...
with tab3:
    st.caption("Ask anything about database design")
    ...
    if question:
        with st.spinner("Searching..."):
            ...
            answer  = generate_sql(openai_client, question, context)
